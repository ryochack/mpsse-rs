...
use mpsse_hal::i2c as mpsse_i2c;
use std::io;

const SLAVE_ADDR: u8 = 0x39;
...
fn power<T, E>(i2c: &mut T, on: bool) -> Result<(), E>
where
    T: hal::blocking::i2c::Write<Error = E>,
{
    let cmd = CommandCode::new(Register::Control).command_bit().build();
    let power = if on { 0x03 as u8 } else { 0x00 as u8 };
    let wdata: [u8; 2] = [cmd, power];
    i2c.write(SLAVE_ADDR, &wdata)?;
...
fn read_data<T, E>(i2c: &mut T) -> Result<(u16, u16), E>
where
    T: hal::blocking::i2c::WriteRead<Error = E>,
{
...
fn main() -> io::Result<()> {
    println!("I2C HAL sample");

    let mut i2c = mpsse_i2c::I2c::new()?;
